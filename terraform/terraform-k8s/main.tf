...
locals {
  ...
  kubeadm_join_cmd = trimspace(data.external.join_command.result["command"])
}
...
data "external" "join_command" {
  program = [
    "${path.module}/get_join_command.sh",
    local.server_node.ssh_key,
    local.server_node.user,
    local.server_node.address,
  ]
